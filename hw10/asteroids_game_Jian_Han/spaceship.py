...
class Spaceship(FlyingObject, object):
    ...
    def control(self, keycode, gc=None):
        ...
        if keycode == 'keyup':
            # x_vel and y_vel will not change
            # spaceship move in its original direction
            self.thrust = False

    def do_thrust(self):
        """Set velocity when thruster is activated"""
        # add the adjustment to x_vel y_vel before
        # call position_and_draw(self, x, y) in FlyingObject
        THRUST_FACTOR = 0.6
